...
### 1-8. 교체 시각 = **임팩트 시각** (`Deadline + SliceTargetImpactOffset`) — (완료) 확정
...
| 경로 | 시각 결정 | 상태 |
|---|---|---|
| 플레이어 칼 | `CharacterActionPlayer.cs:420` — `Deadline + SliceTargetImpactOffset`에 임팩트 정렬 | O 예약 |
| 투사체 절단 | `SliceTargetDirector.cs:205` — `now >= r.impactTime`까지 대기. `Resolve()`는 **성패만** 기록 | O 예약 |
| **적 처치** | `EnemyDirector.cs:396` → `KillOpponent` → `opponent.Kill()` **즉시 실행** | X **예약 없음** |
