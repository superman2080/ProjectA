> **⚠ 폐기됨 (CombatLegibility).** 이 문서가 설계한 무리 모델(`active`/`staged` 두 덩어리 · 집결지 · 승격 ·
> 사망 1 : 스폰 1)은 제거됐다. 지금은 **곡 시작 전에 전원 세우고 벨 때마다 인원이 줄어 무대가 빈다** —
> 근거와 새 모델은 `docs/CombatLegibility/`, 요약은 CLAUDE.md §11-6에 있다.
> **지우지 않고 남긴다** — 왜 만들었고 왜 되돌렸는지가 그 판단의 근거다.
