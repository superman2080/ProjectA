...
- **미오는 질문만 하고 카시마는 딱 한 번을 빼고 되묻지 않는다**(§Narrative 2). 사람의 목소리가 세계 안에서 하는 일이 그 대비 하나뿐이라, 곡이 노래하면 그 규율이 깨진다.
...
- ⚠ **봉인된 균열에서는 이 동기가 통째로 없다.** 일렁임이 없으므로 맞출 것도 없고, 앰비언트의 두 박만 남는다.

#### 무대를 봉인할 때마다 그 자리의 앰비언트가 줄어든다

봉인하는 것은 되돌리는 것이 아니다. 그 자리가 이미 잃은 것이 봉인한 뒤에 드러나며, **가장 먼저 드러나는 채널이 소리다.**

| 무대 | 그 자리의 앰비언트에 거는 것 | 봉인한 뒤 그 자리가 잃는 것 |
|---|---|---|
...
- ⚠ **줄어드는 것은 성적과 무관하다.** `SSS`로 봉인해도 자판기 소리는 똑같이 사라진다. 잘하면 덜 잃는 구조를 만드는 순간 그것이 자책이 된다.
- ⚠ **누적되지만 소급되지 않는다.** 무대 4를 봉인한 뒤 무대 1로 돌아가도 무대 1의 소리는 그때 잃은 만큼만 잃어 있다.
- ⚠ **무대 5는 처음부터 앰비언트가 없다.** 봉인해서 없어지는 것이 아니라 **도착했을 때 이미 없다.** 그것이 그 무대의 신호다.
...
무대를 봉인하고 탐색 씬으로 복귀한 직후, 페이드가 걷히고 1~2초 정적을 둔 다음 한 번 난다. **사건이 일어나는 장면은 보여 주지 않는다.**

| # | 언제 | 소리 | 길이 | 방향 | 신규 자산 |
|---|---|---|---|---|---|
| 1 | 무대 1을 봉인한 뒤 | **저역의 붕괴음 한 번.** 잔향이 길다 | 3초 | **있다.** 다음 무대 쪽 | 1 |
| 2 | 무대 2를 봉인한 뒤 | **저역 럼블 + 금속이 바닥에 닿는 소리.** 셔터가 떨어진다 | 0.6초 | 있다 | 1 |
| 3 | 무대 3을 봉인한 뒤 | **없다.** 대신 골목 앰비언트가 세 겹에서 두 겹이 된다 | — | — | **0** |
| 4 | 무대 4를 봉인한 뒤 | **나무를 한 번 두드리는 소리.** 잔향 없음 | 0.4초 | 있다. 닫힌 문 쪽 | 1 |
...
- ⚠ **봉인하지 못한 균열에서 이 소리들이 반복되지 않는다.** 아직 봉인하지 않은 자리는 일렁임과 그 자리의 −18dB 전자음이 계속 부르고 있다(§6).
...
| | 소리 | 이유 |
|---|---|---|
| **미오** | **18줄.** 호명 응답 1(여섯 번 재사용) + 자발 17. 마지막 하나만 평서문 | §Script 3 |
| **카시마** | **78줄.** ⚠ 사건 6에서만 곡 안 대사가 여덟 줄이다 — 그 곡만 예외로 허용한다 | §Script 2. 그 초과가 곧 그가 통제를 잃었다는 표지다 |
...
| **허물** | **전투 중에는 목소리 0.** 비명·신음이 없고 발소리와 칼 소리만. **소멸하는 순간에만 한 번, 전 게임 여섯 번 말한다**(§Script 4) | 등장 스팅어·전용 모티프는 여전히 없다. 소멸 순간의 말은 **사람 소리가 아니다** — 남자도 여자도 아니고 여러 겹이 겹쳐 누구 것인지 알 수 없다. 마지막 하나만 예외이고 그것은 카시마의 말투다 |
...
#### 일렁이는 균열의 소리 (§Overview 2-4)

- **아직 봉인되지 않은 무대에서는 §Narrative 3-5의 그 −18dB 전자음이 들린다.** 탐색 앰비언트 위에 얹히며, **가까워질수록 커진다**(거리 감쇠 하나. 새 자산 0).
- ⚠ **경고음이 아니다.** 톤·불협·심박·저역 드론 전부 금지 — **판정음 밑에 깔려 있던 그 소리 그대로**여야 한다. 그래야 5스테이지에서 그것이 무엇이었는지 뒤집힐 때 *"내내 들리던 소리"*가 성립한다.
- **봉인된 균열는 완전히 무음이다.** 앰비언트만 남는다 — 그리고 **트루 엔딩으로 가는 세계는 이 소리가 하나도 남지 않은 세계다.**
...
탐색 중에 종이를 읽는 지면이 열려도(§Overview 2-6) **소리가 하나도 바뀌지 않는다.** 열림·닫힘 효과음이 없고, 탐색 앰비언트의 두 박이 그대로 흐르며, 전용 스팅어도 없다.

세계가 자기를 설명하는 동안에도 박은 계속 뛴다 — 그 두 개가 같은 화면에 있는 것이 이 종이들의 뜻이다. 소리를 붙이는 순간 종이는 물건이 아니라 **수집물**이 된다.
...
|---|---|---|
| 무대 곡 | **5** (본전투 1~5) | Suno |
| 짧은 조우 채보 | **7** | 십 초 안팎. 무대별 균열 수는 §Overview 2-1-7 |
