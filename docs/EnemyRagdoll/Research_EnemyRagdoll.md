...
|---|---|---|
| 갈라지는 절단 | 메쉬 분할 + 단면(cap) | ✅ 있음 (`MeshSliceBaker`, 정적 메쉬 사전 굽기) |
| 축 늘어진 시체 | 스켈레톤 + 관절 물리 | ❌ 전무 (`CharacterJoint`/`Ragdoll` 0건) |
...
| 안 | 절단 | 늘어짐 | 단면 | 작업량 | 판정 |
|---|---|---|---|---|---|
| **A. 래그돌만** (절단 포기) | ✗ | ✅ 전신 | — | 최소 | ✗ 절단이 게임의 핵심이라 포기 불가 |
| **B. 셰이더 클립 하이브리드** | △ | ✅ | ❌ **뚫림** | 소 | ✗ **요구사항 위반으로 탈락** |
| **C. 스키닝 보존 절단** | ✅ | ✅ | ✅ | 중 | ✅ **채택** |
| **D. 본 분리** (스켈레톤 자체를 쪼갬) | ✅ | ✅ | ✅ | 최대 | 보류 — MGR급. 저작량이 본질이라 스코프 초과 |
