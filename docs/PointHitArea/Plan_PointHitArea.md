...
- [x] **Step 4 — 검증** (Play 모드에서 실행, 결과 아래 기록)
  ...
  - `Pattern_3Node_(0, 4, 8)` 주입 테스트 결과:
    | 단계 | 결과 |
    | --- | --- |
    | [1] 패턴 없음 (Start 직후) | 9개 모두 `padding = (0,0,0,0)` O |
    | [2] `SetPattern` 후 | Point_1 / Point_5 / Point_9(= 인덱스 0,4,8) `padding = 0`, 나머지 6개 `padding = (25,25,25,25)` O |
    | [3] 패턴 완료 후 | 9개 모두 `padding = (0,0,0,0)`으로 복구 O |
