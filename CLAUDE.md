...
│   │   ├── PauseDirector.cs         # 일시정지 · 포기의 유일 관리 지점(§16)
...
│   │   ├── ChartPlayer.cs           # 채보 시계 + 진행/재시도/종료의 주인(§16)
│   │   ├── StageMode.cs             # enum: Loop/Linear — 종료·재시도·loop 셋을 같이 정한다(§16)
│   │   ├── ChartAuthoring.cs        # enum: Linear/Loop — 저작 방식(툴 전용, 런타임은 안 읽는다)
│   │   ├── Core/                    # OnsetDetector, BeatGrid, OnsetGrouper 등 분석 코어(asmdef)
│   │   ├── Editor/                  # PatternChartWindow(굽기 툴, .Loop.cs = 마디 격자 저작), PatternTemplateLibrary
...
- **⚠ 패턴 실패의 뜻이 `StageMode.Loop`에서 달라졌다**(§16). 판정 자체는 한 줄도 안 바뀌었지만, 공격 패턴을 실패하면 **그 엔트리가 소비되지 않고 다시 나온다** — `PatternHandler`는 여전히 그것을 모르고 `ChartPlayer`가 `OnPatternComplete`를 듣고 정한다.
...
- **`ChartPlayer`**: `GameSession.SelectedChart`(없으면 `debugChart`)를 오디오 재생 시각에 맞춰 순차적으로 `PatternHandler.SetPattern`에 흘려보낸다. **투입 판단은 `audioSource.time`이 아니라 `ChartTime`이다**(§16). 재생 가이드: `docs/!Guides/Guide_ChartPlayback.md`
- **`SongChart.beatsPerBar`(기본 4)는 런타임도 읽는다** — 재시도 되감기를 마디로 올림하는 선택지(§16)와 굽기 툴의 루프 규격 검사가 요구한다. ⚠ **`BeatGrid`는 마디를 모른다**(`subdivisionsPerBeat`만 안다) — 마디 계산은 툴과 런타임이 하고 그 struct는 Unity 비의존 순수 코어로 남는다.
- **`SongChart.authoredWith`(`ChartAuthoring`)는 저작 메타데이터다** — 굽기 툴이 어느 화면으로 열지를 정하고 **런타임은 읽지 않는다**(`patternPool`과 같은 성격). ⚠ **`StageMode`와 다른 물건이다**: 저쪽은 재생 방식, 이쪽은 저작 방식이며 **직교한다**. 기본값이 `Linear`라 기존 채보 17개가 지금 편집기로 열린다.
...
- **굽기 툴은 모드가 둘이다**(`Tools/Pattern Chart Tool` 상단 토글 — **한 창에 담는다**: 창을 둘로 나누면 저장 검증이 복제되고 그중 하나만 고쳐진다). **중간 표현(`ChartEntryDraft`)과 저장 경로(`Save`)를 공유하고 그것을 채우는 방법만 다르다** — 그래서 `SongChart.entries` 스키마가 한 필드도 안 바뀌고 **런타임은 모드를 모른다**.
  - **`Linear`**(현행): 온셋 분석 → 그룹핑 → 라운드로빈 배정. **한 줄도 안 고쳤고 Linear 분기 안으로 격리만 했다** — 삭제하면 기존 채보를 다시 못 굽는다.
  - **`Loop`**(신규, `PatternChartWindow.Loop.cs`): **분석기가 아니라 스텝 시퀀서다.** 마디 격자(가로=스텝, **세로=마디 한 줄** — 한 줄로 늘이면 4마디만 돼도 가로 스크롤만 남는다) 위에 클릭으로 놓고 드래그로 옮긴다. **⚠ 분석이 성립하지 않는 이유**: §16의 재시도 드리프트가 온셋과 음악 트랜지언트의 대응을 지운다 — 대신 **격자 위에만 놓으면 밀려도 여전히 격자 위에 앉는다**.
    - **⚠ 드래프트가 `onsetSteps`(스텝 인덱스)를 진실의 원천으로 든다.** 초를 원천으로 두면 **BPM을 고치는 순간 배치가 격자 밖으로 밀린다**. `onsetSteps == null`이 곧 "이 드래프트는 Linear 것"이라 **별도 플래그가 없다**.
    - **엔트리 = (시작 스텝, 노드 간격, 템플릿)이고 온셋은 `start + i × gap`으로 자동 생성된다.** 그 덕에 격자 위에 있음과 **"노드 수 = 온셋 수"**가 구조적으로 보장돼, Linear에서 계속 나던 *"템플릿 노드 수가 이 그룹과 다릅니다"*가 **표현 불가능**해진다(그 검사는 Linear용으로 남는다). 불규칙 리듬은 행 편집에서 **스텝 단위로** 개별 이동한다.
    - **⚠ `level → subdivision` 매핑을 쓰지 않는다** — 격자 저작에서는 저작자가 해상도(`stepsPerBeat`)를 직접 고르고 `level`은 표시용 난이도로만 남는다.
    - **⚠ 캔버스가 `clip.length`에서 끝나지 않는다** — 루프 2~3바퀴를 쓰는 채보가 정상이라 마디 수를 저작자가 늘릴 수 있고, 곡 1바퀴 경계에 세로선만 긋는다.
    - **미리듣기**: 선택 마디부터 곡 재생 + 캔버스 재생 커서. 분석이 사라진 자리에서 *"이 리듬이 곡과 맞는가"*와 **루프가 균질한가**를 확인하는 유일한 수단이다. ⚠ 에디터 오디오는 공개 API가 없어 **리플렉션이 막히면 조용히 비활성되고 커서만 남는다**. ⚠ **온셋마다 클릭은 못 낸다** — 에디터 미리듣기 채널이 하나뿐이라 클릭이 곡을 잘라 먹는다.
- **저장이 여섯 가지를 검사한다**(`ValidateForSave` — **두 모드 공유**). **지금까지 음악이 공짜로 보장해 주던 것**이라 Linear에서는 실측상 지켜졌을 뿐이고, 격자 저작에서는 즉시 깨진다:
  - **두 모드 모두 중단** ① 빈 채보 ①-b **마지막 엔트리에 `killOnSuccess`가 꺼짐**(그러면 마무리 실루엣이 터지는 프레임에 적이 살아 있다 — 적 수가 처치 수와 같기 때문, §11-6) ② **마지막 엔트리가 통과 패턴**(실패해도 소비되어 마무리 실루엣 없이 끝난다 — ⚠ 이 강제가 그 경우를 **표현 불가능**하게 만들어 런타임 대비 코드가 0줄이다. `Attacker`는 패턴이 소유해 툴이 못 고치므로 자동 보정이 아니라 차단 + 안내다)
  - **Loop에서만 중단 · Linear에서는 경고**(`CheckSpacing`) ③ **인접 엔트리 입력 간격 < 필요치** ④ **큐에 패턴이 셋 오르는 노출**
  - **경고** ⑤ 곡 길이가 마디 정수배가 아니다(**코드로 고칠 수 없는 유일한 위험**이라 여기서만 잡힌다 — 루프마다 음악이 격자에서 밀리고 바퀴마다 누적된다. ⚠ `AudioClip`의 Compression Format 패딩이 원인일 수 있다) ⑥ 음수 스폰 시각
  - **⚠ ③④가 Loop에서 중단인 이유**: 어긴 채보는 *조금 이상한* 게 아니라 **재시도 취소가 하나만 회수하고 나머지를 흘려 적 배정이 영구히 밀린다**(§16).
  - **⚠ 그런데 Linear에서 막으면 안 된다.** 온셋이 음악에서 나오므로 그 값은 **지금까지 강제된 적이 없고 기존 채보에 0.25초 간격이 흔하다** — 막으면 **이미 있는 채보를 열어 한 글자 고치는 것조차 불가능**해진다(회귀 0이 이 개편의 전제다). 격자 저작에서는 반대로 저작자가 방금 놓은 것이라 그 자리에서 고칠 수 있다. **경고는 그대로 찍는다** — 그 채보를 `StageMode.Loop`로 재생하면 증상이 실제로 나므로 조용히 넘기지도 않는다.
  - **⚠ 필요 간격은 상수 0.4초가 아니라 런타임 노브에서 유도한다** — `max(handoverIgnore + 여유, exposure − goodWindow)`. **⚠ 그 값들은 씬의 직렬화 값이고 툴은 씬을 모른다**(§4가 그 의존을 일부러 없앴다) → **같은 기본값을 복제해 두고 검사 메시지에 가정한 숫자를 찍는다**(`chainKillRatio`가 이미 그 방식이다). 그것이 어긋남을 눈으로 보게 하는 유일한 방어다.
- **엔트리 행에 역할 뱃지가 붙는다** — **`재시도`**(실패하면 다시 나온다) / **`통과`**(실패 시 피격하고 넘어간다). `Attacker`가 이제 연출이 아니라 **진행 규칙의 입력**이라 채보 화면에서 읽혀야 한다. 상단에 요약 한 줄(패턴 수 · 재시도/통과 · 마지막 판정 · 꼬리 · 최소 간격 · 마디 수).
- **⚠ `chainKillRatio` 표시는 거짓말이 됐다**(§16 — 사슬 전체가 되감기므로 **모든 타가 결국 성공한다**). 행 뱃지가 `사슬 N/M · 전부 성공해야 처치` + `실패하면 사슬 전체가 다시 나옵니다`로 바뀌었다. **필드 제거는 별개 작업이다**(런타임·씬·툴 세 곳).
- **굽기 툴(상세)**: `PatternChartWindow` — 음원을 온셋 분석(`ChartGen.Core`)해 채보를 굽거나 기존 SongChart를 편집/저장. 가이드: `docs/!Guides/Guide_PatternChartTool.md`. `Core`/`Tests`는 각각 asmdef 보유.
...
- **⚠ 인트로에는 컷으로 들어간다**(`brain.ActiveBlend = null`). **⚠ 한 프레임만 기다리면 안 된다** — Brain이 우선순위 변화를 알아채는 프레임이 호출 시점·실행 순서에 따라 밀려서(탐색 씬에서 넘어오는 경로는 **3프레임 뒤**), 한 프레임 뒤에 끊으면 *아직 없는 것을 끊는* no-op이 되고 그 뒤에 생긴 블렌드가 그대로 끝까지 돈다. 블렌드가 나타날 때까지 최대 `IntroCutGuardFrames`(8) 프레임 지켜본다. 안 그러면 Brain이 게임플레이 vcam에서 인트로로 **들어오는** 블렌드도 만드는데, 그동안 화면 구도는 아직 게임플레이 카메라의 것이라 **스플라인은 달리는데 어깨너머로 정면을 보고 있는** 그림이 된다. `travel` 식은 마무리 블렌드만 창에서 빼므로 들어오는 블렌드까지 같은 값을 먹으면 창이 두 번 깎인다 — 블렌드 2초 · 카운트다운 3초면 **인트로 구도를 한 프레임도 못 본다**. 블렌드가 0.4초일 때는 눈에 안 띄어 오래 숨어 있던 결함이다. 컷이 안 보이는 이유는 곡 시작이 어차피 검은 화면에서 열리기 때문(§9 `ScreenFader`). 상세: `docs/CameraFraming/Research_CameraIntroBlend.md`
...
- **⚠ `Time.timeScale`의 금지에는 경계가 둘 있다**(§14의 마무리 실루엣과 §16의 일시정지). 규칙은 이렇게 읽는다 — **"판정이 남아 있고 오디오가 흐르는 동안"** 못 쓴다. §14는 **판정이 남아 있지 않아서**, §16은 **오디오도 같이 멈춰서** 예외를 얻는다(근거가 서로 다르고, 둘 다 되돌릴 책임만 남는다). 그 둘 밖에서는 아래가 그대로 유효하다:
...
- **⚠ 스테이지가 끝나는 조건은 §16이 든다** — 스토리 모드는 `StageMode.Loop`(모든 패턴을 해결해야 끝난다)이고, 곡 길이로 끝나는 예전 동작은 `Linear`로 남았다. **`GameMode`와 `StageMode`는 직교한다**(§16). 목숨 0 외에 **포기**(`BattleSceneBootstrap.Abandon`)로도 나갈 수 있고, 그쪽도 아무것도 기록하지 않는다.
...
- **잡몹·튜토리얼 전투도 `Encounter`다** — 적 1~2마리(채보의 처치 엔트리 수가 그대로 인원이다, §11-6), 패턴 2~3개, 10초 안팎의 짧은 `SongChart`. **탐색 씬에 전투를 심지 않는다**: 이 게임에서 벤다는 것은 `PatternHandler` 입력 → 판정 → `CharacterActionPlayer` 재생이라 연습용 적 한 마리도 전투 오브젝트 일습을 통째로 요구한다.
...
- **스폰은 화면 밖에서 일어난다**(`PickStagePosition` + `PickSpawnNearCluster`). ⚠ 플레이어가 홱 돌면 방금 나타난 적이 보일 수 있다(감수). 균열 등장(§11-11)이 배선돼 있으면 그쪽이 이기고 절두체 판정은 안 쓴다 — **거기서는 보이는 것이 목적**이라 근거가 반전된다.
- **적은 곡 시작 전에 전원 세워지고 무대 원 안에 흩어진다**(§11-6). 그 자리에서 플레이어를 중심으로 배회한다.
...
### 11-6. 무대의 적 — 전원 미리 세우고, 벨 때마다 준다 (CombatLegibility)
- **적은 곡 시작 전에 <b>전원</b> 태어나 무대 원 안에 흩어져 서고, 플레이어를 중심으로 배회한다.** 그중 지금 패턴이 요구하는 하나만 표적(`currentOpponent`)이 되고 나머지는 예비 적이다. 목록은 **`ring` 하나뿐**이다.
- **⚠ 적 수는 채보가 정한다** — `ChartPlayer`가 `killOnSuccess` 엔트리 수의 합을 세어 `EnemyDirector.PrepareStage(count)`에 넘긴다. **사망 보충이 없으므로 화면 위 인원이 벨 때마다 줄고, 마지막 엔트리에서 무대가 정확히 빈다.** "정리했다"가 화면에 남는 것이 이 모델의 요구다.
  - **⚠ `Count(…)`가 아니라 합으로 적는다**(`ChartPlayer.CountKills`). 값은 지금 같지만 한 엔트리가 여럿을 죽이게 되면(다수 상대) 그 한 항만 바뀐다.
  - **⚠ 마지막 엔트리가 `killOnSuccess`여야 한다**(굽기 툴이 저장을 막는다, §5). 그래야 마무리 실루엣(§14)이 터지는 프레임에 적이 하나도 안 남는다 — **런타임에 맞추는 코드가 0줄**이다.
  - **마름 방지 가드는 `TakeTargetForWindow` 하나뿐이다.** 무대가 비었는데 처치 패턴이 남았으면 한 명 세우고 경고를 찍는다 — 채보와 런타임이 어긋난 경우이지 정상 경로가 아니다.
- **⚠ `StageMode.Loop`에서는 잔여가 구조적으로 0이다**(실패하면 재시도돼 결국 전부 성공한다, §16). `Linear`에서만 실패한 처치 엔트리만큼 남고, 그것은 `ChartPlayer.FinishRoutine`의 `DissolveAll`이 지운다 — **실루엣을 기다린 뒤, 오디오 페이드 앞**이라 소멸이 화면에 보인다.
- **배회는 `ring` 전체가 한다**(`TickWander`, docs/EnemyIdleWander). 지금 상대만 빠진다. **⚠ 궤도 슬롯 수가 `ring.Count`라 인원이 줄면 간격이 벌어진다**(`PickOrbitSlot`이 원주를 그 수로 나눈다) — 곡 후반에 한두 명만 남으면 그 그림이 가장 잘 보이므로, 어색하면 `standoffDistance`를 인원에 따라 줄인다.
- **⚠ 표적에게는 가드 자세를 준다**(`EnemyView.BeginGuard`). 표적이 얻는 시각적 사실이 "배회를 멈춘다" 하나인데 나머지가 계속 도므로 **정지가 배경이 아니라 움직임이 배경**이라 멈춤은 신호가 못 된다. 무연출 폴백 경로(`AssignAttack`/`AssignFeint`에 클립이 없거나 창이 `minFeintWindow`보다 짧을 때)에서 `LookAtInstant` + `guardIdleClip`(`Samurai_BlockIdle`)을 상체 레이어(§11-7)에 얹는다.
  - **⚠ 그 클립은 배회 상체 후보에서 코드가 자동으로 뺀다**(`NextUpperIdleClip`). 비표적이 같은 자세를 쓰면 **"가드 = 지금 표적"의 뜻이 둘로 갈린다.** 에셋을 고치지 않는 이유는 배선이 한 곳(`guardIdleClip`)뿐이라 어긋날 수가 없어서다.
  - 해제는 따로 없다 — `Resolve`의 `reactionUntil`과 `MarkDying`의 `Phase.Dying`을 `TickUpperBody`가 이미 보고, 풀 반납은 `ResetState`가 되돌린다.
  - **⚠ `EnemyFeint`(§11-4)는 손대지 않는다** — 배선돼 있으면 그쪽이 이긴다. 바뀐 것은 **폴백이 Idle에서 가드로** 바뀐 것뿐이라 기존 패턴 회귀가 0이다.
- **스폰 자리는 `PickStagePosition`(무대 원 안, 화면 밖)이고 등장은 `SpawnAt` 하나로 격리돼 있다** — 균열 등장(§11-11)·화면 밖 걸어 들어오기·예산·시선 배선이 전부 그 안이다. `spawnStagger`로 한 명씩 시차를 둔다.
- **⚠ 무리(cluster) 모델은 폐기됐다.** `active`/`staged` 두 덩어리, 집결지(`DesignateStagedCenter`), 승격(`PromoteClusterIfEmpty`), 보충(`ReinforceActiveIfThin`), 사망 1 : 스폰 1이 전부 사라졌다. 이유는 셋이 한 덩어리였기 때문이다 — **인원이 안 줄고**(항상 `clusterSize × 2`), **후보 넷이 2m 안에 뭉쳐** 거리 선택이 임의로 보이며, **집결·승격·보충이라는 세 종류의 이동**이 상시 돌았다. 대가로 거리 이산화가 돌아오지만, 무대 원 전체(반경 8m)에 흩어지므로 오히려 선택지가 무리 모델보다 많다.
- 상세: `docs/CombatLegibility/` · 폐기된 무리 모델: `docs/EnemyCluster/`

### 11-11. 균열 등장 (EnemySpawnWarp)
- **적은 씬의 균열에서 태어나, 균열에 붙은 채 길게 늘어난 줄로 자기 자리까지 끌려 나온다.** 도착하면 그 줄이 몸으로 빨려 들어가며 원형이 된다(레퍼런스: 틱톡 Time Warp Scan). **연출 전용**이고 판정에 개입하지 않으며, 배선이 비면 조용히 예전 경로(화면 밖 스폰)로 돌아간다.
- **균열 목록은 `EnemyDirector.riftPoints`가 든다** — `RiftController`는 무늬와 발광만 미는 순수 연출이라 적을 내는 일을 모른다. 자리에서 **가장 가까운** 균열 하나가 그 적의 등장 지점이고, **꺼진 균열은 후보에서 빠진다**(`Encounter`가 상태에 따라 `SetActive`로 끄므로 §9의 상태 표가 그대로 따라온다).
- **⚠ 이 경로에서는 절두체 판정을 안 쓴다.** 화면 밖 스폰(`EnemyRing.PickSpawnNearCluster` + `spawnMaxOffset`)은 **팝인을 숨기는 장치**였는데 이 연출은 반대로 **보이는 것이 목적**이라 근거가 반전된다 — `SpawnAt` 주석이 예고해 둔 지점이 정확히 여기다. 균열이 없을 때만 그 경로가 남는다.
- **⚠ 길이는 상수가 아니라 균열까지의 실제 거리다.** 상수면(처음에 2m로 뒀다) 9~16m 떨어진 균열에 **닿을 수가 없어** 화면에는 "조금 찌그러진 적이 뛰어온다"만 남는다. `scan = 1`인 정점이 정확히 균열에 앉아야 **줄이 균열에 붙어 있다**가 성립한다.
- **⚠ 공간 변환은 렌더러마다 한다.** 정점은 **그 렌더러의** 오브젝트 공간에 있고 모델 자식은 FBX 보정으로 돌아가 있다(현재 적: X축 270도). 루트 기준으로 축을 넘기면 축이 몸의 **두께 방향**에 얹혀 스캔 좌표가 사실상 상수가 되고, 늘어나는 대신 **몸이 통째로 몇 cm 밀린다**(= 화면에서 아무 일도 안 일어난다). 길이도 같은 변환을 탄다(`InverseTransformVector` — 방향이 아니라 벡터라 모델 스케일이 1이 아니어도 끝이 균열에 앉는다).
- **⚠ 몸 길이를 바운즈로 재면 줄이 끊긴다.** 사람 메쉬는 AABB를 안 채워서 꼬리 정점의 스캔 좌표가 **0.69**에 그쳤고(= 거리의 69%에서 끊김), 그 측정을 **쉬는 포즈**로 하면 여전히 **0.84**다(A포즈가 재생 포즈보다 넓다). `BakeMesh(useScale: false)`로 **지금 포즈**를 굽고 정점 투영의 최소·최대로 재면 0..1에 정확히 펴진다. 작업용 `Mesh`·`List`는 정적으로 재사용해 **스폰마다 할당이 0**이다(§5).
- **구간이 둘이다 — 이동과 수축.** 이동(`spawnWarpDuration`, 기본 1초) 동안 꼬리가 균열에 앉은 채 줄이 길어지고, 도착 뒤 수축(`spawnRetractDuration`, 기본 0.35초)에서 길이가 0으로 줄며 스캔 띠가 꼬리에서 머리로 쓸려 온다. **⚠ 도착과 동시에 놓으면 줄이 끊기는 것으로 보인다** — 그 0.35초가 '원복'의 그림 전부다.
- **연출 중인 적은 아무 일도 맡지 않는다.** `EnemyView.BusyReasonBy`에 한 줄(`"등장 연출 중"`)이면 배회·기습 후보·매 프레임 이격이 **한 술어로 동시에** 빠진다(§11-2·§11-8과 같은 진실의 원천). **상대 배정(`TakeTargetForWindow`)만 따로 막는다** — 거긴 일부러 좁은 술어(`HasPendingAction`)를 써서 이동 중을 안 막기 때문이고, 안 막으면 **늘어난 몸에 칼이 닿는다**. ⚠ 후보가 0일 때의 폴백은 그대로 둔다(상대가 없으면 그 패턴에 벨 대상이 아예 없다).
- **소멸(§11-3 `Dissolve`)의 시간축을 뒤집은 쌍둥이다.** `DissolveSwap` 인스턴스를 **같이 쓴다** — 죽는 것과 태어나는 것은 동시에 일어날 수 없고, 그래서 스왑·원복·**풀 반납 함정(`ResetState`)이 공짜로 따라온다**. ⚠ 평소 입는 툰 머티리얼에는 `_Warp`가 없다 — **갈아끼우는 단계가 있어야 화면에 보인다**.
- **⚠ 프리웜은 `PrepareStage`와 떼어 낼 수 있어야 한다**(`EnemyDirector.PrewarmStage`). 등장을 늦추는 연출(튜토리얼: 미오가 도착하는 순간에 허물이 나온다)에서 `PrepareStage`를 늦추면 **적과 절단 세트의 `Instantiate`가 판정 직전에** 온다(§5). ⚠ `PrefabPool.Prewarm`은 **멱등이 아니라서** `Prewarm`에 `prewarmed` 가드가 있다 — 없으면 두 번째 호출이 인스턴스를 다시 만들고 상한 넘은 만큼 파기해 **피하려던 히치를 그대로 낸다**.
- 튜닝은 `EnemySpawnWarpMaterial` 한 곳(`_EdgeColor`·`_EdgeWidth`)과 디렉터의 시간 노브 둘뿐이다. ⚠ `_WarpStretch`·`_WarpSpan`·`_WarpAxisOS`·`_WarpCenterOS`는 **런타임이 채운다** — 인스펙터 값은 툴 미리보기용이다.
- 상세: `docs/EnemySpawnWarp/`
...
- **⚠ 경고 시간은 상수다**(`minRingExposure` = `maxRingExposure` = **1.0초**). "언제나 같은 시간"이 이 기능의 요구다.
  ...
  - 그래서 **노출을 늘려도 빈도가 안 깎이는 게 아니라 깎인다** — 그 대가는 후보 선정이 되찾는다. 지금 빈도를 붙잡는 유일한 장치는 `cooldown`(3초)이다.
- **후보 선정은 두 번 시도한다** — 사전 접근(`OnDuelScheduled`, 한 패턴 앞)과 늦은 선정(`HandleIdleWindow`). 한 번만 하면 그 순간 조건을 만족하는 적이 없을 때 창이 통째로 날아간다.
  - **후보는 무대에 선 적 전부다**(`PickIdleAmbusher`가 `ring`을 본다). 예전에는 무리로 갈려 있어(`active`/`staged`) 후보가 구조적으로 자주 0이었는데, 무리가 사라지며 그 구분 자체가 없어졌다(§11-6) — 남은 질문은 **거리가 아니라 도착 가능성** 하나다.
  ...
  - **기습이 끝난 적은 제자리에 남는다**(`ReleaseAmbusher`는 예약만 푼다). 다음 프레임에 `TickWander`가 궤도 슬롯을 다시 주므로 돌려보낼 집결지가 없다 — 하나가 튀어나와 찌르고 그 자리에서 다시 섞이는 것이 그 사건의 전부다.
...
- **⚠ 찌르러 서는 자리도 통로를 비켜야 한다**(`EnemyDirector.KeepOutOfDuelLane`). `PlaceAround`는 플레이어와 기습자 자신의 방위만 보는데 **후보를 무대 전체에서 고르므로** 그 방위가 상대 방향과 겹치는 경우가 흔하다 — 그대로 두면 **둘 사이에 낀 채로 선다**. 그리고 기습자는 `hasPendingAttack`이라 매 프레임 이격이 **설계상 건너뛰므로**(찌르러 온 적을 밀면 칼이 빗나간다) **서기 전에 피하지 않으면 아무도 못 치운다**. 통로의 정의와 미는 식은 `EnemyRing.SeparationPush` 그대로 — **새 개념이 0개**다.
  - **⚠ 연타(§2-1)에서 특히 오래 굳는다.** `RaiseIdleWindow`가 공백의 끝을 마무리 일격의 시작으로 잡는데 연타 타격은 **예약이 아니라 입력이 모는 별개 경로**(`HandleMashHit`)라 모델에 안 잡힌다 — 실측 연타 창이 **3.2초**(일반 패턴 노드 스팬 p50 0.8초)라 그 구간이 통째로 "서 있는 공백"으로 신고되고, 기습 임팩트가 창 끝(−0.45초)에 박혀 **면제가 2.7초 지속된다**. 자리를 고칠 뿐 이 오보 자체는 남아 있다(연타 중 기습 빈도가 높은 것이 그 증상이다).
...
- **⚠ 재시도본은 채점되지 않는다**(§16). 그 엔트리의 성적은 **첫 시도로 확정된다** — 재시도는 진행을 위한 것이지 만회 수단이 아니라, **공격 패턴을 실패하면 그 엔트리의 노트는 전부 Miss로 확정된다**(연타의 "초과 타격은 애니메이션만 나오고 점수는 안 오른다"와 같은 관용구).
  - **⚠ 콤보는 갈라진다 — 표시는 다시 쌓이고 점수는 안 쌓인다.** 재시도 구간 내내 화면 콤보가 0에 박혀 있으면 안 되지만, 콤보 풀의 분모는 `N(N+1)/2`로 고정이라 재시도분을 더하면 달성도가 1을 넘는다. §12가 이미 둘을 갈라 놓아서 **구조 변경이 0이다** — `Combo`/`OnComboChanged`(HUD·포스트FX가 보는 표시값)는 오르고 `comboSum`·판정 개수·`successPatterns`는 안 오른다.
  - **⚠ `MaxCombo`도 안 올린다.** 그것은 점수가 아니라 `IsPerfect()`의 `MaxCombo == totalNotes`가 쓰는 값이라 **부풀면 SSS가 영영 안 나온다.**
  - **⚠ 취소된 패턴(§16)은 놓친 노트로 세지 않는다** — 입력 기회가 없었으므로, 세면 재시도마다 벌점이 쌓인다.
  - **⚠ 최종 확정은 한 번뿐이다**(`finalized`) — 완곡 신호가 곡 종료와 마무리 실루엣 종료 두 곳에서 올 수 있다(`BattleSceneBootstrap.resolved`와 같은 성격).
...
### 16. 진행과 재시도 — 스테이지는 곡 길이가 아니라 패턴으로 끝난다 (StageClearCondition)
- **`StageMode`가 셋을 같이 정한다** — 종료 조건 · 재시도 활성 · **`audioSource.loop`**. **⚠ 따로 두면 "`Linear`인데 곡이 루프해서 영영 안 끝나는" 조합이 표현 가능해진다.** 이름이 *곡의 구조*를 가리키고 나머지가 거기서 따라온다.
  - **`Loop`**(기본): 곡이 무한 루프하고, 공격 실패는 재시도하며, **모든 엔트리가 소비되면** 끝난다.
  - **`Linear`**: 곡이 1회 재생되고 재시도가 없으며 **곡이 끝나면** 끝난다 — 예전 동작 그대로다(커스텀 모드용).
  - 진실의 원천은 씬의 `ChartPlayer`이고 `GameSession.StageModeOverride`가 덮는다(`SelectedChart ?? debugChart`와 같은 관용구). **⚠ `SongChart`에 두지 않는다**(같은 곡을 두 모드로 플레이하는 것이 요구다). **⚠ §9의 `GameMode`와 합치지 않는다** — 합치면 "자유 연주는 반드시 `Linear`"가 코드에 박혀 못 뗀다.
- **소비의 기준은 역할이 아니라 "대가를 치렀는가"다.** `소비 = 성공 || Pattern.HitsOnFail`, 그 외는 재시도다.
  - **⚠ `HitsOnFail`은 게터 하나다**(`attacker == Enemy || CountersOnFail`). §6이 *"맞는 경우는 둘뿐"*을 이미 못박아 뒀으므로 **새 구독이 0개**다.
  - **⚠ `CharacterActionPlayer.OnPlayerHit`을 구독해 판단할 수 없다.** 그 이벤트는 `impactTime`에 예약돼 나므로 **`OnPatternComplete`(= `Deadline`)보다 늦다** — 완료 시점엔 아직 대미지가 안 들어와 있어서 *"맞을 예정인데 아직 안 맞았다"*를 재시도로 오판한다.
  - **⚠ 기습 회피(§11-8) 실패로 맞은 것은 여기 안 들어간다** — 패턴 밖의 사건이고 그 패턴의 대가가 아니다. 게터가 `Pattern`만 보므로 그 구분이 **공짜로 성립한다**.
- **재시도 단위는 엔트리가 아니라 사슬이다**(§11-5). 사슬 안에서 하나라도 재시도 사유가 나오면 **머리부터** 되돌린다 — 사슬 중간을 실패하면 그 뒤 타는 취소됐거나 투입도 안 됐고, 적은 안 죽은 채 리액션만 반복한다. 경계는 `killOnSuccess`가 이미 표현하고 있어 **새 저작 데이터가 0개**다(비사슬은 길이 1인 사슬이라 분기가 안 는다).
  - **⚠ 그래서 `chainKillRatio`가 죽은 노브가 됐다** — 사슬 전체가 되감기므로 **모든 타가 결국 성공한다**. 툴이 경고로 알린다(§5). 필드 제거는 별개 작업이다.
- **⚠ 되감는 것은 채보 시계뿐이고 오디오는 손대지 않는다.** `ChartPlayer.chartOffset`을 밀어 `ChartTime`을 그 사슬의 스폰 시각으로 되돌린다 — **뒤따르는 엔트리의 간격이 보존**되어 "재시도 뒤에 전부 쏟아지는" 부류가 원천 소멸한다.
- **⚠ `audioSource.time`은 채보 시계로 못 쓴다** — 그것은 클립 **안의** 위치라 루프마다 0으로 되감기고, 그러면 채보가 곡 2회차에서 처음으로 돌아간다. `ChartPlayer`가 되감김을 세어(`time < prevTime`) `songTime = loops × clipLength + time`으로 **단조 증가**시키고, `ChartTime = songTime − chartOffset`이 투입 판단의 유일한 기준이다.
  - 오디오가 멈추면 `Time.deltaTime`으로 이어 간다. **⚠ `unscaledDeltaTime`이 아니다** — 스폰은 판정(`Time.time`)과 짝이 맞아야 하고, 그 덕에 일시정지에서 채보 시계도 **같이 언다**.
- **⚠ 드리프트는 박의 정수배로만 쌓인다.** 대기를 초가 아니라 **박 수**(`retryGapBeats`, 기본 4박 = 1마디)로 저작하고 되감기 양을 박(또는 마디, `rewindToBar`) 간격으로 **올림**한다 — 그래야 되감긴 온셋이 `BeatGrid` 위에 그대로 앉는다. **곡이 루프라 끝나지 않는 것과 이 둘이 같이 참일 때만 동기화가 유지된다.**
  - **⚠ `bpm <= 0`이면 격자 보존을 포기하고 박 수를 초로 읽는다**(경고 1회). 잠기는 것보다 낫다.
  - **⚠ `retryGapBeats`는 적 방어 클립이 들어갈 창이기도 하다** — BPM 180이면 1.33초, 90이면 2.67초라 **빠른 곡에서 패링 클립이 잘린다**. 적 방어 연출 자체에는 **코드가 0줄**이다: §11-2가 `Attacker.Player` 실패에 넉백 0을 돌려주고 그 0이 곧 패링이라 `Pattern.EnemyParry`가 자동으로 골라진다.
- **취소는 "완료의 한 종류"다**(`PatternCompletionInfo.Cancelled`). 되감기 시점에 이미 큐에 올라간 패턴(리드타임 0.5 > 간격 0.4라 **언제나 최대 1개**)을 회수하는 경로이며, **새 이벤트를 만들지 않는다** — 큐 시점에 상태를 만든 구독자가 전부 이미 `OnPatternComplete`를 구독하고 있어 **새 배선이 0개**다.
  - **⚠ 구독자는 연출을 건너뛴다.** 그냥 실패로 흘리면 **오지도 않은 칼에 적이 패링 모션을 한다.** 발행 쪽도 `AllCorrect = false`로 못박는다 — **손 안 댄 패턴은 그 값이 `true`**(초기값)라 그대로 두면 취소가 처치로 읽혀 적이 갈라진다.
  - **⚠ 취소 대상은 남아 있는 전부다**(선두 포함). 되감기를 결정한 패턴은 `CompletePattern`에서 이미 큐를 떠났고 **그 순간 뒤 패턴이 판정 대상으로 승계됐다**(§3) — 선두를 건너뛰면 취소가 통째로 no-op이 된다.
  - **⚠ 회수 순서는 FIFO다.** 구독자가 자기 큐를 앞에서 꺼내므로 역순으로 발행하면 통지와 예약의 짝이 어긋난다.
  - **⚠ `OnPatternComplete` 디스패치 안에서 부르면 안 된다.** 구독 순서가 보장되지 않아 실패 패턴의 토큰을 아직 못 꺼낸 구독자가 있으면 엉뚱한 예약이 버려진다 — `ChartPlayer`는 핸들러에서 플래그만 세우고 **자기 `Update` 첫머리**에서 부른다.
  - **⚠ `CharacterActionPlayer`도 예약을 끊는다.** 취소될 패턴이 한 프레임 동안 판정 대상이 되므로 `OnJudgeTargetBegan`이 이미 돌아 베기가 예약돼 있다 — 안 끊으면 **허공을 베는 스윙**이 나간다(되감기 간격이 1초 남짓이라 눈에 그대로 보인다). ⚠ **이미 시작한 재생은 안 끊는다**(§6의 "휘둘렀는데 막혔다"와 같은 결이고, 끊으면 `swingActive`가 내려가 히트스톱 가드와 트레일이 스윙 도중에 꺼진다).
  - **⚠ `EnemyView.AbortPendingAction`은 `Resolve`와 다른 물건이다.** `Resolve`는 실패에서 **반드시** 패링/회피로 크로스페이드한다. 다만 **`Phase.Windup`은 빠져나와야 한다** — `ApplyLocomotion`이 그 상태에서 스스로 물러나므로 예약만 지우면 **적이 영구히 갇힌다**(§11-10이 겪은 부류).
- **⚠ 재시도 표식(`IsRetry`)은 사슬이 아니라 엔트리별로 매긴다.** 뜻은 *"그 엔트리가 `OnPatternComplete`를 실제로 한 번이라도 겪었는가"*이고 **취소는 겪은 것으로 치지 않는다**(입력 기회가 없었다). 사슬 단위로 매기면 사슬 뒤쪽 엔트리가 **첫 시도를 해 본 적도 없이 영원히 채점 불가**가 된다.
- **종료는 상태기 하나다** — `cleared`(남은 엔트리 0 · 진행 중 0 · `PatternHandler.HasActivePatterns` false) → `outroHold`(1.5초, `goodWindow` + `ImpactOffset` + 절단 + 히트스톱 + 여운) → **마무리 실루엣이 안 바쁠 때**(§14의 `IsBusy`) → 오디오 페이드(`outroFade`) → `DissolveAll` → `OnSongEnded`.
  - **⚠ 이벤트를 새로 만들지 않았다.** 종점은 계속 `OnSongEnded` 하나이고 바뀐 것은 *언제 쏘는가*뿐이라 `ScoreDirector`·`BattleSceneBootstrap`의 구독이 한 줄도 안 바뀐다.
  - **⚠ 루프곡에서 `audioSource.Stop()`은 선택이 아니다** — `loop = true`면 곡이 스스로 끝나지 않으므로 **이 경로가 유일한 정지 지점이다**.
  - **⚠ `ChartPlayer.Stop()`을 재사용하지 않는다** — 그쪽은 `ClearAllPatterns`를 부르고 그것이 `OnAllPatternsCleared`로 **실루엣을 죽인다**. 여기서는 오디오만 멈춘다(큐는 이미 비어 있다).
  - **⚠ 페이드는 `audioSource.volume`에 직접 대입하지 않는다** — `MusicPlayerBase.ApplyMusicVolume`이 볼륨의 주인이라 페이드 도중 `OnVolumeChanged`가 오면 값이 되돌아간다. `VolumeFactor`(파생이 곱하는 계수) 하나로 둘이 서로를 덮지 않는다.
  - **⚠ 실루엣은 `outroHold`에 합치지 않는다** — `holdDuration`이 더 길 수 있고, 합치면 **실루엣이 없는 실패 경로가 이유 없이 길어진다**.
- **⚠ 등급 기록을 한 프레임 미룬다**(`BattleSceneBootstrap.ReportAndReturn`). `session.LastResult`는 `ScoreDirector`가 채우는데 두 클래스가 같은 이벤트를 구독하고 **호출 순서가 씬 오브젝트 순서**라, 먼저 불리면 **직전 곡의 등급**이 박힌다. `outroHold`는 *완료 뒤의 여유*이지 **같은 호출 흐름 안의 순서를 바꾸지 않는다** — 둘은 다른 문제다. `resolved` 가드는 코루틴 밖에서 **즉시** 세운다.
- **일시정지·포기는 `PauseDirector`가 든다**(`UI/`, `CameraDirector`와 같은 관례 — 배선이 비면 조용히 비활성). **무한 재시도가 설계상 정상이라 나가는 수단이 필수다** — 없으면 실력이 모자란 플레이어가 그 무대에 갇힌다.
  - **⚠ `Time.timeScale = 0`과 `audioSource.Pause()`를 같이 건다.** §7-3의 금지 근거는 *"게임 시계만 느려지고 오디오는 안 느려져 차이가 영구 누적된다"*인데 **오디오도 같이 멈추면 그 차이가 생기지 않는다** — §14가 "판정이 남아 있지 않다"로 얻은 것과 **다른 근거로** 얻는 예외다. **둘 중 하나만 걸면 안 된다.**
  - **⚠ 실루엣 중에는 일시정지를 막는다**(`IsBusy` 재사용). §14가 이미 `timeScale = 0.1`을 걸어 둔 상태라 해제가 그것을 1로 덮는다 — **복구 주체가 둘이 되는 문제를 진입을 막아 없앤다**(새 상태가 0개).
  - **⚠ `OnDisable`에서 되돌린다.** 정지 중에 꺼지면 게임이 영구히 0배속으로 굳는다.
  - **포기는 `BattleSceneBootstrap.Abandon()`이고 `HandleDepleted`와 같은 처리다** — 곡을 끊고 **아무것도 기록하지 않는다**(§9: 중단에서 등급이 새어 나가면 `Fresh`가 `Retry`로 바뀐다). **새 분기가 0개다.**
- **⚠ 튜토리얼과 절대 겹치지 않는다.** 종료 판정·재시도 판단은 `Play()`를 거친 경우에만 돌고(`pendingEntries == null`이면 `Update` 첫 줄에서 return), 드릴(`PatternDrillStep`)은 자체 재시도를 계속 쓴다.
- **⚠ 굽기 툴과 런타임이 짝이다** — "취소는 언제나 최대 1개"라는 전제가 엔트리 간격·노출에 기대는데 **그 값은 지금까지 강제된 적이 없다**(온셋이 음악에서 나와 실측상 그랬을 뿐이다). §5의 저장 차단이 그것을 강제한다.
- **⚠ 오토플레이(§8)는 실패 경로를 못 만든다** — 이 시스템의 검증에 쓸 수 없다.
- 상세: `docs/StageClearCondition/` · 격자 저작: `docs/LoopChartTool/`
...
- **⚠ 취소·재시도본은 완료 횟수에 안 들어간다**(§16). 세면 재시도 몇 번으로 총량에 먼저 도달해 **곡 중반에 실루엣이 터지고**, 그러면 timeScale 예외의 근거("판정이 남아 있지 않다")가 통째로 깨진다.
- **⚠ `ChartPlayer`가 이 연출이 끝나기를 기다린다**(`IsBusy`). `armed`도 봐야 한다 — `active`만 보면 완료~임팩트 사이(0.1초)에 스테이지 종료가 끼어들어 **실루엣이 시작조차 못 한다**.
...
- **서사 설계의 진실의 원천은 `docs/Story/`의 여섯 문서이고, 그중 `Story_Events.md`가 서사 진행과 용어에 관해 나머지를 이긴다** — `Story_Overview.md`(구조: 층·엔딩·시스템 연결) / `Story_Narrative.md`(인물·장소·사건) / `Story_Script.md`(대사·이미지·샷·자산) / `Story_Music.md`(곡) / `Story_Playthrough.md`(**손과 화면만** — 사건 순서는 여기 없다) / `Story_Events.md`(**사건 여덟 — 플레이어가 겪는 순서와 그때마다 바뀌는 질문. 전체 흐름을 볼 때 여기부터 읽는다**). **CLAUDE.md는 "어떻게 보이는가", Story 문서는 "왜 그렇게 보이는가"**를 다룬다 — 둘이 겹치면 성질에 맞는 쪽으로 보낸다. (설계 과정의 Research/Plan 문서들은 확정 후 폐기했다.)
- **용어**: 무대가 서 있는 지점은 **균열**, 그 균열의 허물을 전부 소멸시키는 것은 **봉인한다**이다(구 `자리`·`닫는다`는 폐기). ⚠ **봉인은 가둬 두는 말이라 소멸이 아니다** — 세계의 교리와 단어가 어긋나 있고 아무도 지적하지 않는다(`Story_Events.md`).
- **진행은 여덟 사건이다**(`Story_Events.md`). 사건마다 모양이 다르고, 끝날 때마다 플레이어가 품는 질문이 바뀐다. ⚠ **카시마는 사건 0~2에서 진짜로 다정하고**, 격앙은 한 사건에 한 단씩만 오른다. 그의 물음표는 전 대본에서 하나(사건 4), 느낌표는 사건 6에만 있다.
...
- **가짜 흑막이 있다** — 「면을 쓴 자」(`Story_Overview.md` §4-3). 4스테이지 씬 **안의 두 번째 무대(4-b)**에서 그를 베지만 아무것도 끝나지 않는다. 그가 요구하는 **신규 코드는 0개**다 — 적 수가 채보에서 나오므로(§11-6) "한 씬에 `Encounter`가 둘이라 인원을 씬 단위로 못 둔다"는 문제가 사라졌고(`clusterSizeOverride`는 그래서 제거됐다), 나머지는 전부 기존 파라미터(`killOnSuccess` 사슬 · `DeathSliceSet` 조각 교체 · 이격 규칙)다. ⚠ **그에게 목소리·발광·아웃라인·전용 스팅어를 주지 않는다.**
- **⚠ 서사 진행과 전투는 같은 공간·같은 흐름 안에 있다**(`Story_Overview.md` §1-1) — 곡 선택 씬과 전투 씬을 오가던 구조는 스토리 모드에서 폐기됐다. **§9가 그 구조를, §11-2가 그 유일한 위험 지점(위치 소유권)을 다룬다.** 여기서는 그것이 서사에 요구하는 것만 적는다:
  ...
  - **읽히는 것은 종이뿐이다** — 탐색 중에 상호작용해 읽는 문건 일곱 장(`Story_Overview.md` §2-6, 원문은 `Story_Script.md` §10-A). ⚠ **도상 쪽 프롬프트 금지는 이것 때문에 느슨해지지 않는다** — 규칙이 종류로 갈린다(글씨가 적힌 것만 읽힌다). 근접 표시는 기능 안내가 아니라 **그 종이에 적힌 제목 한 줄**이며(`Retry` 프롬프트와 같은 관용구), 수집률·목록·획득음이 없다. 신규 코드는 근접·상호작용 컴포넌트 **하나**뿐이다 — 지면 뷰는 트루 엔딩의 서류철 다섯 장이 이미 요구한다.
