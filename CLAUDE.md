...
- **스폰은 자기 자리에서 가장 가까운 화면 밖 지점**이다. 무대 가장자리에서 걸어오게 하면 첫 이동만 무대 횡단(최대 16m)이 되어 재배치 예산으로 감당할 수 없다. 자리가 이미 화면 밖이면 **오프셋 0 = 이동 없음**이 정상 경로다. 등장 연출은 `SpawnAt` 하나로 격리돼 있어 **그 메서드만 갈아끼우면 된다** — **⚠ 실제로 갈아끼웠다(§11-11): 균열이 배선돼 있으면 이 절의 화면 밖 스폰은 폴백이고, 적은 균열에서 늘어나 나온다.**
...
- **⚠ 찌르러 서는 자리도 통로를 비켜야 한다**(`EnemyDirector.KeepOutOfDuelLane`). `PlaceAround`는 플레이어와 기습자 자신의 방위만 보는데 **후보는 현재 상대와 같은 무리에서 고르므로**(§11-6) 그 방위가 곧 상대 방향인 경우가 흔하다 — 그대로 두면 **둘 사이에 낀 채로 선다**. 그리고 기습자는 `hasPendingAttack`이라 매 프레임 이격이 **설계상 건너뛰므로**(찌르러 온 적을 밀면 칼이 빗나간다) **서기 전에 피하지 않으면 아무도 못 치운다**. 통로의 정의와 미는 식은 `EnemyRing.SeparationPush` 그대로 — **새 개념이 0개**다.
